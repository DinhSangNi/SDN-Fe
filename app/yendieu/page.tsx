...
export default function SimpleProfile() {
  const info = {
    avatar: '/avatars/yendieu.jpg',
    position: "Student",
    description: "Đam mê ăn mì cay ",
    name: 'Đỗ Yến Diệu ',
    age: 18,
    address: 'Quảng Ngãi ',
    email: 'dieunek7@gmail.com',
    phone: '0123 456 789',
    hobbies: ['ăn mì cay ', 'Đọc sách', 'Chơi game', 'Du lịch']
  };
    ...
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-blue-50 to-purple-100 p-4">
      <div className="bg-white shadow-2xl rounded-2xl overflow-hidden max-w-lg w-full p-8 space-y-6">
        ...
        <div className="text-center">
          <img src={info.avatar} alt="Ảnh đại diện" className="w-32 h-32 mx-auto rounded-full border-4 border-blue-200 hover:scale-110 transition-transform duration-300" />
        ...
        <div className="text-sm text-gray-700">
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {info.hobbies.map((hobby, index) => (
              <span key={index} className="bg-blue-200 text-blue-800 px-3 py-1 rounded-lg text-xs">{hobby}</span>
            ))}
          </div>
        </div>
        {/* Mạng xã hội */}
